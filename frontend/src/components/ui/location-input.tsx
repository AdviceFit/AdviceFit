...
const LocationSelector = ({
  ...
  const availableStates = statesData.filter(
    (state) =>
      ...
      (selectedCountry?.name ?? "").toLowerCase() ===
        state.country_name.toLowerCase()
  );
  ...
  useEffect(() => {
    setSelectedCountry(
      countriesData.find((country) => value && country.name == value[0]) ?? null
    );
    setSelectedState(
      statesData.find((state) => value && state.name == value[1]) ?? null
    );
  }, []);
  ...
  return (
    <>
      ...
      <div className="flex gap-2">
        ...
        {(formState && !selectedState && availableStates.length > 0) ? (
          <span className="text-red-500 text-[0.8rem]">State is required</span>
        ) : null}
      </div>
    </>
  );
